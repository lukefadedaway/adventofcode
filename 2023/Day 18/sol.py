...
def calculate_polygon_area(coordinates):
    ...
    area = 0.5 * sum((coordinates[i][0] * coordinates[(i + 1) % n][1] - coordinates[(i + 1) % n][0] * coordinates[i][1]) for i in range(n))
    return abs(area)
...
solution = (calculate_polygon_area(coordinates) + 1 - border // 2) + border
solution2 = (calculate_polygon_area(coordinates2) + 1 - border2 // 2) + border2
